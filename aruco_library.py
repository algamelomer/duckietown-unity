...
def detect_and_draw_markers(frame, aruco_type, camera_matrix, dist_coeffs, marker_size_meters=0.05):
    """
    Detects ArUco markers in a single image frame, draws annotations, and returns the results.
    
    Args:
        frame: The input image.
        aruco_type: The ArucoType enum value for the dictionary.
        camera_matrix: The camera calibration matrix.
        dist_coeffs: The distortion coefficients.
        marker_size_meters: The actual physical size of the marker in meters.
        
    Returns:
        A tuple containing:
        - The annotated frame with drawings.
        - The list of detected marker IDs.
        - The list of translation vectors (tvecs) for each marker.
    """
    ...
    # Important for accurate pose estimation with solvePnP
    ...
    tvecs_list = [] # Store tvecs for returning
    ...
    if ids is not None: 
        ...
        for i, corner in enumerate(corners): 
            # Use solvePnP for each marker to get rvec and tvec
            # This is the method from your original code that works
            ret, rvecs, tvecs = cv2.solvePnP(world_points, corner, camera_matrix, dist_coeffs)
            tvecs_list.append(tvecs)
            
            position = tvecs.flatten()
    ...
    return frame, ids, tvecs_list, corners
